...
def modify_configuration(variable_name, value):
    if hasattr(robot, variable_name):
        setattr(robot, variable_name, value)
        print(
            f"Configuration updated: {variable_name} = {getattr(robot, variable_name)}")
    else:
...
def run_simulation():
    ...
    return test_best_distance, test_time
...
def noise_testing():
    ...
    noise_values = [0, 0.25, 0.5, 0.75, 1]
    decay_values = [0.25, 0.5, 0.75]
    num_runs = 3
    all_results = []

    for initial_noise in noise_values:

        modify_configuration('INITIAL_NOISE', initial_noise)

        for noise_decay in decay_values:

            modify_configuration('NOISE_DECAY', noise_decay)

            for run in range(num_runs):
                print(
                    f"Running simulation for intial noise: {initial_noise}, iteration {run + 1}")

                test_best_distance, test_time = run_simulation()

                result = {'initial_noise': initial_noise,
                        'noise_decay': noise_decay,
                        'run': run,
                        'test_best_distance': test_best_distance,
                        'test_time': test_time}
...
# sim_random_shooting_path_length()
noise_testing()